...
router.post('/submit', authenticateAnonymous, function (req, res, next) {
  // todo: ip checks
    ...
    RegistrationModel.find({ email: user.email }, function (err, docs) {
      if (docs.length === 0) {
        ...
        return res.sendStatus(401, {
          error: 'A user with this email has been removed from the system',
        });
      } else {
        return res.sendStatus(401);
      }
...
router.post('/complete-registration', tokenValidate, function (req, res, next) {
  try {
    const authHeader = req.headers['authorization'];
    const token = authHeader && authHeader.split(' ')[1];
...
router.post('/authenticate', authenticateAnonymous, function (req, res, next) {
  const authHeader = req.headers['authorization'];
  const token = authHeader && authHeader.split(' ')[1];
    ...
    try {
      ...
      UserModel.find({ email }, function (err, docs) {
        if (err || docs.length == 0)
          return res.sendStatus(500, { error: 'No email specified.' });
...
router.post('/token', authenticateAnonymous, function (req, res, next) {
  ...
  try {
    ...
    UserModel.findById(decrypted._id, function (err, doc) {
      if (err) {
        error(
          'Password reset token auth failed | user not found',
          token,
          null,
          err
        );
