...
router.post(
  '/checkout',
  (req, res, next) =>
    authenticateToken(req, res, next, [ROLES.CIPHER, ROLES.DEFAULT_USER]),
  function (req, res, next) {
    ...
    try {
      ...
      if (!articleIds || !Array.isArray(req.body) || articleIds.length === 0)
        ...
        return async.parallel(
          [
                ...
                [
                  (callback) => {
                    ...
                    articleIds.forEach((articleId) =>
                      promises.push(findArticle(articleId))
                    );
                  ...
                  (articles, callback) => {
                    ...
                    articles.forEach((article) =>
                      articlePromises.push(findFiles(article))
                    );
            ...
            (cb) => {
              ...
              let promises = [];
              articleIds.forEach((articleId) =>
                promises.push(updatePurchase(articleId))
              );
            ...
            (purchases, cb) => {
              async.parallel(
                ...
                (err, result) => {
                  ...
                  let promises = [];
                  purchases.forEach((purchase) =>
                    promises.push(mapEntries(purchase))
                  );
